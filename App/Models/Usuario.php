...
class Usuario extends Model
{
    ...
    public static function getRecursosActivos()
    {
        $usuario_id = Session::get("user_id");
        
        $sql = "SELECT r.controlador, r.accion " 
             . "FROM recurso r " 
             . "WHERE id IN ("
             . "    SELECT recurso_id " 
             . "    FROM perfil_recurso "
             . "    WHERE perfil_id IN (" 
             . "        SELECT perfil_id FROM usuario WHERE id = :usuario_id ) " 
             . "    AND activo = 1 ) " 
             . "AND r.activo = 1 ORDER BY r.controlador, r.accion";
        $lista_recursos = Db::findAll($sql, [":usuario_id" => $usuario_id]);
